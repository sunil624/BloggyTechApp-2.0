...
const PostDetails = () => {
  ...
  // const {comments} = useSelector((state)=> state?.posts?.posts?.allposts);
  ...
  const isLiked = post?.post?.likes?.includes(userAuth?.userInfo?._id);
  const isDisliked = post?.post?.dislikes?.includes(userAuth?.userInfo?._id);
          ...
          <div className="container px-4 mx-auto">
            <div className="mx-auto mb-12 text-center md:max-w-2xl">
             ...
             {post?.post?.content}
              ...
              <Link to={`/users/public-profile/${post?.post?.author?._id}`}>
